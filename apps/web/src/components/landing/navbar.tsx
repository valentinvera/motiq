  ...
  return (
    <>
      <div className="flex flex-1 justify-start">
        <Link
          className="group flex items-center gap-2.5 font-bold text-white text-xl tracking-tighter"
          to="/"
        >
      ...
      <nav className="hidden items-center gap-6 md:flex">
        {links.map(({ href, name }) => (
          <a
            className="font-medium text-sm text-white/50 transition-colors hover:text-white"
            href={href}
            key={name}
          >
            {name}
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-x-0 top-20 z-[60] border-white/5 border-b bg-zinc-950/95 p-6 shadow-2xl backdrop-blur-2xl md:hidden"
            exit={{ opacity: 0, y: -10 }}
