...
- **tRPC data flow**: `apps/api` → mounts `appRouter` from `@motiq/trpc/routers` → `apps/web` consumes via `@trpc/tanstack-react-query` with superjson transformer.
- **Auth flow**: API handles auth via Better Auth middleware. Web app uses `authClient` from `@/lib/auth-client` for client-side auth.
