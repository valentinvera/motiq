# Motiq — AI Context & Code Standards

This file defines the architectural and coding standards for the **Motiq** project.
**Stack:** Bun, Turborepo, TanStack Start (Web), Hono (API), tRPC, Drizzle ORM, Better Auth, Tailwind CSS v4, Shadcn UI.
...
bun run dev:web          # Start only the frontend (TanStack Start on port from VITE_PORT)
bun run dev:api          # Start only the API (Hono on port 3000)
...
### Type Safety
- **Zod:** Use Zod schemas for all API inputs (tRPC) and env validation.
- **No `any`:** Use `unknown` and narrow with Zod.
- **Explicit returns:** Public API functions should have explicit return types for tRPC inference.
...
- **Data Fetching:** Use tRPC via `useTRPC()` hook or Server Functions.
- **React 19:** Use `ref` as a prop. Do not use `forwardRef`.
- **Images:** Use standard `<img>` tags (NOT Next.js `<Image>`).

### Backend: Hono + tRPC
- **Runtime:** Bun.
- **Routers:** Define tRPC routers in `packages/trpc/src/routers/`.
- **Procedures:** Use `publicProcedure` and `protectedProcedure` from `@motiq/trpc`.
- **Context:** Keep context lightweight (Auth Session + DB).

### Database: Drizzle ORM
- **Schema:** Defined in `packages/db/src/schema/*.ts`.
- **Querying:** Prefer the Query API (`db.query.table.findMany`) over raw SQL.
- **Migrations:** Never modify the database manually. Use generate + migrate commands.
...
- Dark mode handled via CSS variables (`oklch`) and standard Tailwind classes (`dark:`).
...
1. **Connect** — Universal intake: Zendesk, Freshdesk, Typeform, Google Forms, Gong/Chorus call recordings, product analytics events, social mentions, email inboxes (support@, feedback@). Auto-syncs every 5 minutes, no code required.

2. **Monitor** — Autonomous AI agents running 24/7:
...
3. **Act** — Autonomous actions: auto-routes tickets to the right team, creates Jira issues with full context, sends Slack escalations for critical signals, drafts proactive CSM outreach for churn risks, delivers role-based daily digests.
...
**Note to AI:** When generating code, follow the architecture defined above. All packages use tsdown and export compiled `.mjs` files. Use Shadcn + Tailwind v4 for UI. Never use `process.env` directly — always import from `@motiq/env`.
